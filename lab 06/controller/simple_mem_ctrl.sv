module simple_mem_ctrl #(
    ...
    // Master 0 (CPU)
    input  logic [ADDR_WIDTH-1:0] addr0, //base address of transaction (word address)
    input  logic [DATA_WIDTH-1:0] wdata0, //Write data (valid when we=1)
    output logic [DATA_WIDTH-1:0] rdata0, //Read data (valid when we=0)
    input  logic                  we0, //Write enable (1=write, 0=read)
    input  logic                  req0, //Request valid
    output logic                  gnt0, // This master has been granted, request to be processed

    // Master 1 (DMA)
    input  logic [ADDR_WIDTH-1:0] addr1, //base address of transaction (word address)
    input  logic [DATA_WIDTH-1:0] wdata1, //Write data (valid when we=1)
    output logic [DATA_WIDTH-1:0] rdata1, //Read data (valid when we=0)
    input  logic                  we1, //Write enable (1=write, 0=read)
    input  logic                  req1, //Request valid
    output logic                  gnt1  // This master has been granted, request to be processed
);
