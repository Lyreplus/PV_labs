...
The second stimulus covers the \texttt{went\_full} and \texttt{full\_bin} bins because we fully populate the FIFO, with 16 writes, and the last stimulus covers the \texttt{illegal} bin because we assert both \texttt{wr\_en} and \texttt{rd\_en} at the same time, which causes the FIFO to ignore both operations (neither write nor read is performed).
...
\begin{itemize}
  ...
  \item Offset (addr[OFFSET\_WIDTH-1 -: OFFSET\_WIDTH]): the least significant bits of the address, used to select the specific word within the cache line.
\end{itemize}
