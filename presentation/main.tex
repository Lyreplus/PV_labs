...
Verification process faces significant challenges:
        ...
        \item<3-> Executing all possible sequences is often infeasible % due to time and resource constraints.
        \item<4-> Identification of the most effective test sequences: crucial for efficient testing.
    \end{itemize}
\end{frame}
...
\begin{frame}{Current Verification Approaches}
    ...
    \begin{itemize}
        ...
        \item<3-> Encounters state space explosion problem for large scale designs. 
    \end{itemize}
    \textbf{Simulation-based verification}:
    \begin{itemize}
        \item<4-> High scalability and applicability.
        \item<5-> Effectiveness rely on well-designed constraints.
        \item<6-> Biased constraints: miss critical scenarios.
        \item<7-> Strict constraints: increase debugging complexity.
    \end{itemize}
\end{frame}
...
RISC-V Torture Test:
    \begin{itemize}
        \item Generate random instruction sequences to achieve faster coverage closure
        \item Lacks structured verification guidance: limited effectiveness in extreme corner cases
    \end{itemize}
RISCV-DV:
    \begin{itemize}
        \item Support CRV (constraint random verification) and RISC V ISA features
        \item Reliance on randomness makes it difficult to cover boundary conditions
    \end{itemize}
Fuzzing tools: cannot guarantee coverage of all possible scenarios.
\end{frame}
...
\begin{frame}{Overview}
    \begin{enumerate}
        ...
        \item<3-> Formal Cover Property Generation
...
% formal verification with realistic system behavior.
    \item<1-> Primary Inputs (PIs) such as reset, clock, interrupt request, rvalid signal for AXI protocol. 
    \begin{itemize}
        \item<2-> Treated as free nets: assume arbitrary values without constraints.
        \item<2-> Incorrect coverage analysis: generated cover traces may fail to address coverage holes.
        \item<2-> Unconstrained PIs can result in misleading coverage estimation. E.g.:  test scenarios correspond to unreachable states by design (AXI protocol).
        \item<2-> Formal verification may fail to generate meaningful instruction patterns.
...
\begin{frame}{BBOX (Black Box) Constraints}
\begin{itemize}
    \item<1-> \textbf{Definition}: In EDA tools, modules with high storage requirements or complex computational logic are treated as BBOX, causing false alarms and verification gaps.
    \item<2-> BBOX modules cannot correctly drive outputs, they may become free nets, preventing module logic from driving its outputs.
    \item<3-> \textbf{Solution}: Abstraction models preserve critical behavior while ensuring interaction with the system remains verifiable.
    \item<4-> \textbf{High Storage Example}: Cache abstraction using SVA array to track indices. A single predefined tag replaces full tag tracking, reducing overhead while preserving cache functionality.
    \item<5-> \textbf{Computation Example}: Symbolic computation approach using SVA to approximate multiplier outputs, capturing input signals and using simplified results for consistency verification.
\end{itemize}
...
\begin{frame}{Insufficient RISCV-DV Constraints}
\begin{itemize}
    \item<1-> \textbf{Problem}: Formal verification may produce specialized memory data sequences that existing constraints in RISCV-DV cannot properly handle.
    ...
    \item<5-> \textbf{Result}: Coverage holes related to multi-level page table accesses can now be successfully filled.
\end{itemize}
...
\begin{itemize}
    \item<1-> \textbf{Overview}: Use formal cover properties to obtain cover traces and generate instruction sequences addressing target coverage holes.
    ...
    \item<3-> \textbf{Formal Execution}: Formal tool executes the cover property and derives the input patterns required to satisfy the condition.
    \item<4-> \textbf{Cover Trace Generation}: Generates corresponding cover trace from the formal verification results.
    \item<5-> \textbf{Sequence Extraction}: Extract the instruction sequence from the trace to effectively cover the coverage hole.
\end{itemize}
...
\begin{frame}{Cache Abstraction Model Codification}
    \begin{figure}
        \centering
        \includegraphics[width=0.85\textwidth]{cache abstraction code.png}
...
\begin{itemize}
    \item<1-> \textbf{Unreachable Coverage Holes}: First review whether constraints in both formal and simulation environments are overly restrictive.
    \item<2-> \textbf{Waiving Unreachable Holes}: If constraints are appropriate and hole is unreachable, it is waived; information still provided to designers for debugging reference.
    \item<3-> \textbf{Resource Exhaustion}: State space explosion is problematic for large-scale designs and limits formal tool capabilities.
    \item<4-> \textbf{Decomposition Strategy}: Complex properties are decomposed into smaller sub-properties to collect their cover traces independently.
    \item<5-> \textbf{Integration}: Integrate sub-property traces into comprehensive instruction sequence, ensuring manageable complexity while maintaining verification workability.
\end{itemize}
