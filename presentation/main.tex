...
RISC-V Torture Test:
    \begin{itemize}
        \item<2-> \emoji{thumbs-up} Generate random instruction sequences to achieve faster coverage closure
        \item<2-> \emoji{thumbs-down} Lacks structured verification guidance: limited effectiveness in extreme corner cases
    \end{itemize}
RISCV-DV:
    \begin{itemize}
        \item<3-> \emoji{thumbs-up} Support CRV (constraint random verification) and RISC V ISA features
        \item<3-> \emoji{thumbs-down} Reliance on randomness makes it difficult to cover boundary conditions
    \end{itemize}
Fuzzing tools
\begin{itemize}
    \item<4-> \emoji{thumbs-down}Cannot guarantee coverage of all possible scenarios.
\end{itemize}

\end{frame}

\begin{frame}{Problems \& Issues}
\begin{alertblock}{Problems \& Issues}
    \begin{itemize}
...
\begin{frame}{Overall architecture}
    \begin{figure}
        \centering
        \includegraphics[width=0.9\textwidth]{FGTSO_arch.png}
...
% comprehensive design verification.
 Priority: addressing \textbf{most impactful coverage holes} % by focusing on the ones that affect verification efficiency, accelerating the verification process.
 \begin{pblock}{MaxUncovd strategy}
     Prioritizes selecting modules with lowest coverage, assuming that addressing the most uncovered areas yields the greatest verification impact. % MaxUncovd enhances the overall efficiency and ensures comprehensive design verification.
 \end{pblock}   
...
\begin{alertblock}{Problem}
    Modules with high storage requirements or complex computational logic cannot be fully modeled in formal verification, leading to BBOX constraints.
\end{alertblock}
E.g.: for a cache: huge amount of cache lines. Tracking each line is infeasible.
\begin{itemize}
    \item <2-> 
...
\begin{frame}{FGTSO Workflow Misalignment - RISCV-DV Constraints}
    % create a box for the problem item, should be red
    \begin{alertblock}{Problem}
        Formal verification may produce sequences that RISCV-DV constraints cannot handle.
    \end{alertblock}
\begin{itemize}
...
\begin{frame}{Comparison with Other Frameworks}
    \begin{figure}
        \centering
        \includegraphics[width=0.75\textwidth]{framework comparison.png}
